/* 
 * libartificial - Small header-only C library for arbitrarily deep Artificial Neural Networks
 * 
 * Copyright (c) 2020 Fuzznets Private Company and Dim Karoukis. All rights reserved.
 * 
           ...
           double X[(*rows) * (*cols_X)])
{
  ...
  for (i = 0; i < (*rows); i++)
  {
    for (j = 0; j < (*cols_Y); j++)
    {
      Y[i * (*cols_Y) + j] = 2 * pow(X[i * (*cols_X) + 0], 2) + X[i * (*cols_X) + 1] * 3 - X[i * (*cols_X) + 2] + rand_normal(0, 1);
    }
  }
}
...
int main(void)
{
  ...
  const char funcs[2][30] = {
      "tanh",
      "linear" // Regression
  };
  //

  // Normalize X for the gradients
